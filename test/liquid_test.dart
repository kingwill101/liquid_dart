...
void main() {
  group('tests', () {
    ...
    test('render', () async {
      final context = Context.create();
      final root = TestRoot({
        'basic': '{% render "name.html" %}',
        'basic_with_vars': '{% render "name_age.html", name, age:1 %}',

        'name_age.html': '{{ name }}! {{age}}',
        'name.html': '{{ name }}!',
        'name_snippet.html': '{{ greeting }}, {{ name }}!',
        'with': '{% render "name_snippet.html"}'
      });

      context.variables['name'] = 'John';
      context.variables['greeting'] = 'Hello';

      var template = Template.parse(context, await root.resolve('basic'));
      
      template = Template.parse(context, await root.resolve('basic_with_vars'));
      expect(await template.render(context), equals('John! 1'));
